...
export default function AdminPage() {
  ...
  <h1 className="text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-yellow-300 to-yellow-600">Centro de Monitoreo Root</h1>
          ...
          {alertas.length > 0 && (
            <div className="mb-8 bg-zinc-900 border border-zinc-800 shadow-[0_0_30px_rgba(0,0,0,0.5)] rounded-xl p-6 shadow-sm">
              <h2 className="text-xl font-bold text-white mb-4 flex items-center gap-2">🔔 Alertas y Avisos ({alertas.length})</h2>
              <div className="space-y-3">
                {alertas.map(alerta => (
                  <div key={alerta.id} className="bg-yellow-50 border border-zinc-800 p-4 rounded-lg flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div>
                      <span className="text-[10px] bg-yellow-500/20 text-yellow-400 font-bold px-2 py-0.5 rounded uppercase tracking-wider mb-1 inline-block">{alerta.tipo.replace('_', ' ')}</span>
                    ...
                    <button onClick={() => marcarLeida(alerta.id)} className="bg-zinc-900 border border-zinc-850 hover:border-zinc-800 text-xs font-bold text-zinc-400 px-3 py-1.5 rounded transition-colors shadow-sm whitespace-nowrap">
                      ✓ Marcar Leída
                    </button>
                  </div>
                ))}
          ...
          <div className="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-4">
            <div className="p-6 bg-zinc-950 border border-zinc-800 rounded-lg flex flex-col">
