        ...
        <div className="max-w-7xl mx-auto">
          <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8 border-b border-zinc-800 pb-6">
            <div className="flex items-center gap-4">
              <div className="bg-[#fe5000]/10 p-3.5 rounded-2xl border border-[#fe5000]/30 shadow-lg">
            ...
            <Link href="/admin" className="text-xs bg-[#181920] border border-zinc-800 hover:border-yellow-500 text-yellow-400 hover:text-yellow-300 px-4 py-2.5 rounded-xl transition-all font-bold shadow-md">
          ...
          {cargando ? (
             ...
             <div className="bg-[#181920] border border-zinc-800 p-12 rounded-2xl text-center shadow-sm">
                <p className="text-zinc-500 text-sm">No se ha generado ninguna comisión aún en el sistema.</p>
             </div>
          ) : (
             <div className="space-y-5">
                {afiliadosEmails.map(email => {
                   ...
                   return (
                      <div key={email} className="bg-[#181920] border border-zinc-800 rounded-2xl shadow-xl overflow-hidden transition-all">
                         <div onClick={() => setExpandedEmail(isExpanded ? null : email)} className="cursor-pointer p-5 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 hover:bg-zinc-800/40 transition-colors">
                            <div className="flex items-center gap-3.5">
                               <div className="bg-[#121316] border border-zinc-800 p-3 rounded-2xl text-2xl shadow-inner">👤</div>
                               <div>
                                  <h3 className="font-bold text-white text-lg flex items-center gap-2">{email}</h3>
                                  <p className="text-xs text-zinc-400 mt-0.5">{items.length} comisiones registradas</p>
                               </div>
                            </div>
                            ...
                            <div className="flex items-center gap-6 w-full md:w-auto justify-between md:justify-end border-t md:border-t-0 border-zinc-800 pt-3 md:pt-0">
                               <div className="text-left md:text-right">
                                  <p className="text-[10px] text-zinc-500 font-bold uppercase tracking-widest">Saldo Pendiente</p>
                                  <p className={`text-xl font-black ${totalPendiente > 0 ? "text-red-400" : "text-zinc-500"}`}>
                                    ${totalPendiente.toLocaleString("es-AR")}
                                  </p>
                               </div>
                               <div className="text-right">
                                  <p className="text-[10px] text-zinc-500 font-bold uppercase tracking-widest">Total Abonado</p>
                                  <p className="text-lg font-bold text-green-400">${totalPagadoHistorico.toLocaleString("es-AR")}</p>
                               </div>
                         ...
                         {isExpanded && (
                            <div className="p-5 border-t border-zinc-800/80 bg-[#121316]/60 space-y-6">
                               <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 bg-[#181920] p-4 rounded-xl border border-zinc-800">
                                  <div>
                                    <h4 className="font-black text-sm text-white uppercase tracking-wider flex items-center gap-2">
                                      💳 Registrar Pago a Vendedor
                                    </h4>
                                    <p className="text-xs text-zinc-400">Elegí cancelar la totalidad o realizar entregas a cuenta</p>
                                  </div>
                                  ...
                                  {totalPendiente > 0 ? (
                                    <div className="flex flex-wrap gap-2 w-full sm:w-auto">
                                      <button 
                                        ...
                                        className="flex-1 sm:flex-none bg-yellow-500/10 hover:bg-yellow-500/20 border border-yellow-500/40 text-yellow-400 px-4 py-2.5 rounded-xl text-xs font-bold transition-all shadow-md flex items-center justify-center gap-1.5"
                                      >
                                      ...
                                      <button 
                                        ...
                                        className="flex-1 sm:flex-none bg-green-600 hover:bg-green-500 text-white px-5 py-2.5 rounded-xl text-xs font-black transition-all shadow-lg flex items-center justify-center gap-1.5 uppercase tracking-wider"
                                      >
                                        <CheckCircle2 className="w-4 h-4" /> Liquidar Total (${totalPendiente.toLocaleString("es-AR")})
                                      </button>
                                    </div>
                                  ) : (
                                    <span className="bg-green-500/10 text-green-400 border border-green-500/20 text-xs px-3 py-1.5 rounded-lg font-bold flex items-center gap-1.5">
                                      ✅ Sin Saldo Pendiente
                                    </span>
                                  )}
                               </div>
                               ...
                               {pendientes.length > 0 && (
                                  <div className="space-y-2.5">
                                     <h5 className="text-xs font-black text-amber-400 uppercase tracking-widest flex items-center gap-1.5 border-b border-zinc-800 pb-1.5">
                                        <AlertCircle className="w-3.5 h-3.5 text-amber-400" /> Comisiones Pendientes de Liquidar ({pendientes.length})
                                     </h5>
                                     <div className="space-y-2">
                                        {pendientes.map(p => {
                                           ...
                                           return (
                                              <div key={p.id} className="bg-[#181920] border border-zinc-800 p-3.5 rounded-xl flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 text-xs shadow-sm">
                                                 <div className="space-y-0.5">
                                                    <p className="font-bold text-white text-sm">
                                                       {p.clienteNombre ? "Cliente: " + p.clienteNombre : "Venta General"} 
                                                       <span className="text-zinc-500 font-normal text-xs ml-2">({new Date(p.fecha).toLocaleDateString("es-AR")})</span>
                                                    </p>
                                                 ...
                                                 <div className="text-right flex items-center gap-3">
                                                    ...
                                                    <div className="text-right">
                                                       <span className="font-black text-amber-400 text-sm bg-amber-500/10 px-2.5 py-1 rounded-lg border border-amber-500/20 font-mono">
                                                         ${saldoItem.toLocaleString("es-AR")}
                                                       </span>
                                                       <p className="text-[9px] text-zinc-500 mt-1">Original: ${p.comisionAsociada}</p>
                                                    </div>
                                                 </div>
                                              </div>
                                           );
                               ...
                               {pagosPreviosAfiliado.length > 0 && (
                                  <div className="space-y-2.5 pt-2">
                                     <h5 className="text-xs font-black text-blue-400 uppercase tracking-widest flex items-center gap-1.5 border-b border-zinc-800 pb-1.5">
                                     ...
                                     <div className="space-y-2">
                                        {pagosPreviosAfiliado.map(pago => (
                                           <div key={pago.id} className="bg-[#181920] border border-blue-950 p-3.5 rounded-xl flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 text-xs shadow-md">
                                              <div className="space-y-1">
                                                 <div className="flex items-center gap-2">
                                                    <span className={`text-[9px] px-2 py-0.5 rounded font-black uppercase ${pago.tipoPago === "TOTAL" ? "bg-green-500/20 text-green-400 border border-green-500/30" : "bg-yellow-500/20 text-yellow-400 border border-yellow-500/30"}`}>
                                                 ...
                                                 <p className="text-white font-bold">
                                                    Método: <span className="text-zinc-300 font-normal">{pago.metodoPago} ({pago.cuentaOrigen || "Caja General"})</span>
                                                 </p>
                                              ...
                                              <div className="text-right flex items-center gap-3 w-full sm:w-auto justify-between sm:justify-end border-t sm:border-t-0 border-zinc-800 pt-2 sm:pt-0">
                                                 <div>
                                                    <span className="text-green-400 font-black text-base font-mono">
                                                      +${(pago.montoPagado || 0).toLocaleString("es-AR")}
                                                    </span>
                                                    <p className="text-[9px] text-zinc-500">Saldo Restante: ${pago.saldoRestante?.toLocaleString("es-AR")}</p>
                                                 </div>

                                                 <button 
                                                   type="button"
                                                   onClick={() => generarComprobantePDF(email, pago, [])}
                                                   className="bg-zinc-800 hover:bg-zinc-700 text-zinc-300 hover:text-white font-bold p-2 rounded-lg text-[10px] border border-zinc-700 transition-colors flex items-center gap-1"
                               ...
                               {pagadasCompletas.length > 0 && (
                                  <div className="space-y-2.5 pt-2">
                                     <h5 className="text-xs font-black text-green-400 uppercase tracking-widest flex items-center gap-1.5 border-b border-zinc-800 pb-1.5 opacity-80">
                                        <CheckCircle2 className="w-3.5 h-3.5 text-green-400" /> Comisiones Totalmente Liquidadas ({pagadasCompletas.length})
                                     </h5>
                                     <div className="space-y-2 opacity-75">
                                        {pagadasCompletas.map(p => (
                                           <div key={p.id} className="bg-[#181920] border border-zinc-800 p-3 rounded-xl flex justify-between items-center text-xs">
                                              <div>
                                                 <p className="font-bold text-zinc-300">
                                                    {p.clienteNombre ? "Cliente: " + p.clienteNombre : "Venta General"} 
                                                    <span className="text-zinc-500 font-normal ml-2 text-[10px]">({new Date(p.fecha).toLocaleDateString("es-AR")})</span>
                                                 </p>
                                              ...
                                              <div className="text-right">
                                                 <span className="font-bold text-green-400 font-mono">${p.comisionAsociada}</span>
                                                 {p.fechaPagoAfil && (
          ...
          {modalPagoOpen && (
            <div className="fixed inset-0 bg-[#121316]/85 backdrop-blur-md flex items-center justify-center z-50 p-4 overflow-y-auto">
              <div className="bg-[#181920] border border-zinc-800 rounded-3xl w-full max-w-lg p-6 md:p-8 space-y-6 shadow-2xl relative">
                
                <div className="flex justify-between items-center border-b border-zinc-800 pb-4">
                  <div>
                    <span className="text-[9px] bg-[#fe5000]/20 text-[#fe5000] font-black uppercase px-2.5 py-1 rounded border border-[#fe5000]/30 inline-block mb-1">
                      REGISTRO DE LIQUIDACIÓN
                    </span>
                    <h3 className="text-lg font-black text-white">
                      {pagoTipo === "TOTAL" ? "Liquidación Total de Comisiones" : "Pago Parcial a Cuenta"}
                    </h3>
                    <p className="text-xs text-zinc-400 font-bold mt-0.5">Vendedor: {pagoAfiliadoEmail}</p>
                  </div>
                  <button 
                    onClick={() => setModalPagoOpen(false)} 
                    className="text-zinc-400 hover:text-white font-black text-sm bg-zinc-800 p-2 rounded-xl"
                  >
                    ✕
                  </button>
                </div>
                ...
                <form onSubmit={handleEjecutarPagoComisiones} className="space-y-4 text-xs">
                  ...
                  <div className="grid grid-cols-2 gap-2 bg-[#121316] p-1.5 rounded-xl border border-zinc-800">
                    <button
                      type="button"
                      onClick={() => {
                        setPagoTipo("TOTAL");
                      ...
                      className={`py-2 rounded-lg font-bold text-xs transition-all ${pagoTipo === "TOTAL" ? "bg-green-600 text-white shadow-md font-black" : "text-zinc-400 hover:text-white"}`}
                    >
                      💚 Liquidación Total
                    </button>
                    ...
                    <button
                      ...
                      className={`py-2 rounded-lg font-bold text-xs transition-all ${pagoTipo === "PARCIAL" ? "bg-yellow-500 text-black shadow-md font-black" : "text-zinc-400 hover:text-white"}`}
                    >
                      ✏️ Pago Parcial
                    </button>
                  </div>
                  ...
                  <div>
                    ...
                    <input 
                      ...
                      className="w-full bg-[#121316] border border-zinc-700 p-3 rounded-xl text-white font-mono font-black text-base outline-none focus:border-yellow-500 shadow-inner"
                    />
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div>
                      ...
                      <select 
                        ...
                        className="w-full bg-[#121316] border border-zinc-800 p-2.5 rounded-xl text-white font-bold outline-none focus:border-yellow-500"
                      >
                    ...
                    <div>
                      ...
                      <input 
                        ...
                        className="w-full bg-[#121316] border border-zinc-800 p-2.5 rounded-xl text-white outline-none focus:border-yellow-500 font-bold"
                      />
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <input 
                      ...
                      className="w-full bg-[#121316] border border-zinc-800 p-2.5 rounded-xl text-white font-mono outline-none focus:border-yellow-500"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <textarea 
                      ...
                      className="w-full bg-[#121316] border border-zinc-800 p-2.5 rounded-xl text-white outline-none focus:border-yellow-500 resize-none h-16"
                    />
                  </div>
                  ...
                  <div className="pt-2 border-t border-zinc-800 flex gap-2">
                    <button 
                      ...
                      className="flex-1 bg-yellow-500 hover:bg-yellow-400 text-black font-black py-3 rounded-xl text-xs uppercase tracking-wider transition-all shadow-xl disabled:opacity-50"
                    >
                    ...
                    <button 
                      type="button" 
                      onClick={() => setModalPagoOpen(false)}
                      className="bg-zinc-800 hover:bg-zinc-700 text-zinc-400 font-bold px-4 py-3 rounded-xl text-xs"
                    >
                      Cancelar
                    </button>
                  </div>
