...
export default function ClientePage() {
  ...
  if (loading || cargandoDatos) {
    return <div className="min-h-screen bg-black text-yellow-500 p-8 text-center mt-20">Cargando tu perfil...</div>;
  }
  ...
  if (!user.emailVerified && user.email !== "jpmosqueiramo@gmail.com") {
    return (
      <div className="min-h-screen bg-black text-yellow-500 p-8 flex flex-col items-center justify-center">
        <div className="bg-zinc-900 border border-yellow-500/30 p-10 rounded-lg text-center max-w-lg shadow-[0_0_20px_rgba(234,179,8,0.2)]">
    ...
    <div className="min-h-screen bg-black text-yellow-500 p-8">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-8 grid grid-cols-1 md:grid-cols-2 gap-4">
          <button 
            onClick={() => window.location.href = `https://wa.me/5491125659686?text=${encodeURIComponent("Hola Cuenta Hogar, necesito asistencia técnica, avisar de un pago o informar un atraso. Mi email es: " + user.email)}`}
            className="bg-zinc-900 border border-yellow-500/30 p-4 rounded-lg flex items-center gap-4 hover:border-yellow-500 hover:-translate-y-1 active:scale-95 transition-all shadow-md"
          >
             <div className="text-4xl">🛠️</div>
             <div className="text-left">
               <h3 className="font-bold text-yellow-400 text-lg">Asistencia y Pagos</h3>
               <p className="text-sm text-gray-400">Informar pagos, atrasos o pedir servicio técnico.</p>
             </div>
          </button>
          
          <button 
            onClick={() => window.location.href = `https://wa.me/5491125659686?text=${encodeURIComponent("Hola Cuenta Hogar, necesito actualizar mis datos personales / contacto. Mi email es: " + user.email)}`}
            className="bg-zinc-900 border border-yellow-500/30 p-4 rounded-lg flex items-center gap-4 hover:border-yellow-500 hover:-translate-y-1 active:scale-95 transition-all shadow-md"
          >
             <div className="text-4xl">📝</div>
             <div className="text-left">
               <h3 className="font-bold text-yellow-400 text-lg">Actualizar mis Datos</h3>
               <p className="text-sm text-gray-400">Avisar si cambiaste de número o de domicilio.</p>
             </div>
          </button>
        </div>

        {solicitudes.length > 0 && !mostrarFormulario && (
        ...
        {(solicitudes.length > 0 && !mostrarFormulario) ? (
          <div className="space-y-6">
            <h2 className="text-2xl font-semibold mb-4 text-white">Tus Solicitudes Actuales</h2>
            {solicitudes.map(sol => (
              <div key={sol.id} className="bg-zinc-900 border border-yellow-500/30 rounded-lg p-6">
                <div className="flex justify-between items-start mb-4">
                  <div>
                    ...
                    <p className="text-sm text-gray-400 mt-1">Enviado el {sol.fechaCreacion?.toDate().toLocaleDateString()}</p>
                  </div>
                ...
                {(sol as any).estadoEntrega === "ENTREGADO" && (
                  <div className="mt-5 p-4 bg-zinc-800/50 border border-zinc-700 rounded-lg">
                    <h4 className="text-sm font-bold text-gray-300 mb-2">🏷️ Anticipo de Entrega</h4>
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                       <span className="text-xl font-black text-white">${(sol as any).montoAbonado || "0"} <span className="text-xs text-gray-500 uppercase font-normal ml-1">({(sol as any).metodoPago || "Efectivo"})</span></span>
                ...
                {(sol as any).estadoEntrega === "ENTREGADO" && (sol as any).planPagos && (
                  <div className="mt-6 pt-6 border-t border-yellow-500/20">
                    ...
                    <p className="text-gray-400 text-sm mb-6">Aquí puedes subir y reportar las transferencias o recibos mensuales de tus cuotas.</p>
                    <div className="space-y-4">
                      {(sol as any).planPagos.map((cuota: any, idx: number) => (
                        <div key={idx} className="bg-black border border-yellow-500/20 p-4 rounded-lg flex flex-col md:flex-row md:items-center justify-between gap-4 shadow-md">
                          <div>
                            <p className="font-bold text-white text-lg">Cuota {cuota.numero} de {(sol as any).planElegido}</p>
                            <p className="text-gray-400 text-sm">Vencimiento: {new Date(cuota.vencimiento).toLocaleDateString()}</p>
                          ...
                          <div className="text-right flex flex-col justify-center">
                            ...
                            {cuota.estado === "EN_REVISION" && (
                              <div className="flex flex-col items-end gap-2 bg-blue-900/20 p-3 rounded border border-blue-500/30">
                                ...
                                <p className="text-[10px] text-gray-400 max-w-xs text-right">El administrador de Cuenta Hogar está revisando el recibo que subiste en la cuenta bancaria.</p>
                              </div>
                            )}
                            
                            {cuota.estado === "PENDIENTE" && (
                              <div className="flex flex-col items-end gap-3 bg-zinc-900 border border-zinc-800 p-4 rounded">
                                ...
                                <input type="file" id={`comprobante_${sol.id}_${idx}`} accept="image/*,application/pdf" className="text-[11px] w-full max-w-xs text-gray-400 file:bg-yellow-500 file:text-black file:border-0 file:rounded file:px-3 file:py-1.5 file:font-bold hover:file:bg-yellow-400 file:cursor-pointer outline-none" />
                                <button 
                                  onClick={async () => {
          ...
          <div className="bg-zinc-900 border border-yellow-500/20 rounded-lg p-6 lg:p-10 relative">
            {solicitudes.length > 0 && (
               <button onClick={() => setMostrarFormulario(false)} className="absolute top-6 right-6 text-gray-400 hover:text-white font-bold text-sm">
                 ✕ Cancelar
               </button>
            )}
            <h2 className="text-2xl mb-2 font-bold text-white">Solicitar Nuevo Crédito</h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-black p-4 rounded-lg border border-yellow-500/10">
                <div>
                  <label className="block text-sm mb-2 text-yellow-400 font-semibold">Nombre Completo</label>
                  <input required value={nombreCompleto} onChange={e=>setNombreCompleto(e.target.value)} type="text" placeholder="Juan Perez" className="w-full bg-zinc-800 border border-gray-700 rounded p-3 text-white focus:border-yellow-500 focus:outline-none" />
                </div>
                <div>
                  <label className="block text-sm mb-2 text-yellow-400 font-semibold">Número de DNI</label>
                  <input required value={numeroDni} onChange={e=>setNumeroDni(e.target.value)} type="number" placeholder="Ej: 32444555" className="w-full bg-zinc-800 border border-gray-700 rounded p-3 text-white focus:border-yellow-500 focus:outline-none" />
                </div>
                <div>
                  <label className="block text-sm mb-2 text-yellow-400 font-semibold">Teléfono / Celular</label>
                  <input required value={telefono} onChange={e=>setTelefono(e.target.value)} type="tel" placeholder="Ej: +54 9 11 1234-5678" className="w-full bg-zinc-800 border border-gray-700 rounded p-3 text-white focus:border-yellow-500 focus:outline-none" />
                </div>
                <div>
                  <label className="block text-sm mb-2 text-yellow-400 font-semibold">Dirección</label>
                  <input required value={direccion} onChange={e=>setDireccion(e.target.value)} type="text" placeholder="Ej: Av. San Martin 123" className="w-full bg-zinc-800 border border-gray-700 rounded p-3 text-white focus:border-yellow-500 focus:outline-none" />
                </div>
                <div>
                  <label className="block text-sm mb-2 text-yellow-400 font-semibold">Localidad</label>
                  <input required value={localidad} onChange={e=>setLocalidad(e.target.value)} type="text" placeholder="Ej: Córdoba Capital" className="w-full bg-zinc-800 border border-gray-700 rounded p-3 text-white focus:border-yellow-500 focus:outline-none" />
                </div>
                <div>
                  <label className="block text-sm mb-2 text-yellow-400 font-semibold">¿Qué producto deseas financiar?</label>
                  <input required value={producto} onChange={e=>setProducto(e.target.value)} type="text" placeholder="Ej: Heladera Samsung 400L" className="w-full bg-zinc-800 border border-yellow-500/50 rounded p-3 text-white focus:border-yellow-500 focus:outline-none" />
                </div>
              </div>
